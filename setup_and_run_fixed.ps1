# PowerShell Setup Script - اصلاح‌شده
# این اسکریپت همه وابستگی‌ها را نصب می‌کند و اینترنت را دور می‌زند

Write-Host "=== Starting setup_and_run.ps1 ==="
...
if (-not (Test-Path "$env:USERPROFILE\.git-credentials")) {
    ...
    $cred = ConvertFrom-SecureString $password
    Set-Content "$env:USERPROFILE\.git-credentials" "https://$username:$cred@github.com"
